...
case $mode in
    c | C)
        ...
        if [[ -f "tmp/new_$file.tmp" ]]; then
            ...
            sort tmp/new_$file.tmp | uniq -c | sort -nr | nl > result/$filename
        ...
        echo -e "$(wc -l "result/$filename" | tr " " '\n' | head -n 1) distinct words"
        ;;
    ...
    p | P)
        ...
        if [[ -f "tmp/new_$file.tmp" ]]; then
            sort tmp/new_$file.tmp | uniq -c | sort -nr | nl | head -n $n > result/$filename
            #Após a listagem da orrência de palavras os ficheiros são tmp são eliminados
            rm tmp/new_$file.tmp && rm tmp/$file.tmp
        else
        ...
        printf "set terminal png\nset output \"result/$filename.png\"\nset boxwidth 0.5\nset style fill solid\nplot \"result/$filename\" using 1:2:xtic(3) with boxes" > bar.gnuplot
        gnuplot < bar.gnuplot
        display result/$filename.png
        printf "<img src=\"result/$filename.png\">" > result/$filename.html
        ;;
